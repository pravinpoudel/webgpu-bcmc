...
fn main(@builtin(global_invocation_id) g_invocation_id : vec3<u32>) {
    ...
    //if the ray does not contribute for isosurface
    ...
    */
    ...
    let n_blocks = volume_params.padded_dims.xyz / uint3(4u);
    //if this ray contain the iso-surface block it is active
    ...
    if (num_rays == 1u) {
        ...
        //normalized cordinate because here bounding volume is not 1x1x1 rather voxel size is considered 1x1x1
        //unlike in raycaster demo; volume_scale is default to [1, 1, 1]
        ...
        //again normalizing aganist bounding volume size
        var radius = 3.5;
