...
// outputs array contain the index of buffer at that index 
    if (inputs[gl_GlobalInvocationID.x] != 0) {
            outputs[offsets[gl_GlobalInvocationID.x]] = gl_GlobalInvocationID.x + compact_offset;
    }
 */

// THIS IS FOR COMPACTING BLOCKS
StreamCompact.prototype.compactActiveIDs = async function (
  numElements,
  isActiveBuffer,
  offsetsBuffer,
  outputBuffer
...
//THIS IS FOR COMPACTING RAYS
StreamCompact.prototype.compactActive = async function (
  numElements,
  isActiveBuffer,
  offsetsBuffer,
  dataBuffer,
